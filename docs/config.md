...
## AI features (nfty fork)
nfty — the AI fork of ntfy — can optionally use an LLM provider for AI features: natural-language
subscription setup, message enrichment/digests, chat over notification history, and an MCP server for
AI agents (see [the fork plan](ai-plan/index.md) for the roadmap and phases).

!!! warning
    All AI features are **off by default**. With AI disabled (the default), nfty behaves identically
    to upstream ntfy.

**Privacy:** the recommended setup is a **local provider** (e.g. [Ollama](https://ollama.com)) so that
message content never leaves your server:
...
Cloud providers (`openai`, `anthropic`, `poolside`, or any `openai-compatible` endpoint such as OpenRouter/vLLM)
require an API key. Every outbound AI call is logged locally (feature, model, token counts, duration)
so you can audit exactly what was sent, and every request counts against per-visitor and global daily
...
| `ai-enabled`                               | `NTFY_AI_ENABLED`                               | *boolean* (`true` or `false`)                       | `false`           | AI (nfty fork): Enables the AI layer and the `/v1/ai/*` endpoints. With AI disabled, nfty behaves identically to upstream ntfy. See [AI features](#ai-features-nfty-fork).                                                               |
| `ai-provider`                              | `NTFY_AI_PROVIDER`                              | *string*                                            | -                 | AI: Provider, one of `openai`, `anthropic`, `openai-compatible`, `ollama`, `poolside`, `mock`. Required if `ai-enabled` is set.                                                                                                          |
| `ai-base-url`                              | `NTFY_AI_BASE_URL`                              | *URL*                                               | provider default  | AI: Provider API base URL, e.g. `http://localhost:11434/v1` for Ollama. Defaults per provider.                                                                                                                                           |
| `ai-api-key`                               | `NTFY_AI_API_KEY`                               | *string*                                            | -                 | AI: Provider API key. Not required for local providers (e.g. Ollama).                                                                                                                                                                    |
| `ai-api-key-file`                          | `NTFY_AI_API_KEY_FILE`                          | *filename*                                          | -                 | AI: File containing the provider API key (alternative to `ai-api-key`)                                                                                                                                                                   |
| `ai-model`                                 | `NTFY_AI_MODEL`                                 | *string*                                            | provider default  | AI: Default model for all AI features, e.g. `llama3.1`, `gpt-4o-mini`, `claude-3-5-haiku-latest`                                                                                                                                         |
| `ai-model-plan`                            | `NTFY_AI_MODEL_PLAN`                            | *string*                                            | -                 | AI: Model override for subscription planning                                                                                                                                                                                             |
| `ai-model-enrich`                          | `NTFY_AI_MODEL_ENRICH`                          | *string*                                            | -                 | AI: Model override for message enrichment                                                                                                                                                                                                |
| `ai-model-digest`                          | `NTFY_AI_MODEL_DIGEST`                          | *string*                                            | -                 | AI: Model override for digests                                                                                                                                                                                                           |
| `ai-model-chat`                            | `NTFY_AI_MODEL_CHAT`                            | *string*                                            | -                 | AI: Model override for chat over notification history                                                                                                                                                                                    |
| `ai-embeddings-model`                      | `NTFY_AI_EMBEDDINGS_MODEL`                      | *string*                                            | -                 | AI: Embeddings model for semantic chat retrieval (e.g. `nomic-embed-text` on Ollama). Empty = keyword-only retrieval. Requires an embeddings-capable provider (Ollama, OpenAI-compatible; not Anthropic).                                  |                                                                                                                                                                                    |
...
OPTIONS:
   ...
   --ai-embeddings-model value, --ai_embeddings_model value                                                                       embeddings model for semantic chat retrieval, e.g. nomic-embed-text (empty = keyword-only) [$NTFY_AI_EMBEDDINGS_MODEL]
